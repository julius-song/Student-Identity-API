...
def launch_api(classifier_name, host, port):
    ...
    def test():
        ...
        try:
            ...
            return json.dumps(json.loads(
                                    features.to_json(orient = 'records')),
                             indent = 4)
        except:
            return '''<h1>Error when processing features</h1>'''
    ...
    def model():
        ...
        weights0_list = []
        for val in weights0:
            weights0_list.append(list(map(float, list(val))))
            
        bias0_list = list(map(float, classifier.get_variable_value("dnn/hiddenlayer_0/bias")))
        
        weights1 = classifier.get_variable_value("dnn/hiddenlayer_1/kernel")
        weights1_list = []
        for val in weights1:
            weights1_list.append(list(map(float, list(val))))
            
        bias1_list = list(map(float, classifier.get_variable_value("dnn/hiddenlayer_1/bias")))
        
        logit_weights = classifier.get_variable_value('dnn/logits/kernel')
        logit_weights_list = []
        for val in logit_weights:
            logit_weights_list.append(list(map(float, list(val))))
            
        logit_bias_list = list(map(float, classifier.get_variable_value('dnn/logits/bias')))
        
        results = {'variable_names': variable_names, 'weights0': weights0_list, 'bias0': bias0_list, 
                   'weights1': weights1_list, 'bias1': bias1_list, 'logit_weights': logit_weights_list, 
